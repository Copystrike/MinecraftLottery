...
/**
 * YamlFile - This class will make creating configs so much easier
 *
 * @author Copystrike
...
public class YamlFile extends ConfigFile {
     ...
     */
    public YamlFile(LotteryBase plugin, String name) {
    ...
    public void createFile() {
        ...
        if (!getFile().getParentFile().mkdir() && !getFile().getParentFile().exists()) {
            getPlugin().getLotteryLogger().warning("Could not find parent directory!");
            return;
        }
    ...
    public void save() {
        try {
            ...
            getPlugin().getLotteryLogger().info("Failed to save file! Reason: " + e.getMessage());
        }
    }
